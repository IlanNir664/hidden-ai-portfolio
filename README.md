...
[Methodology](hidden_ai_portfolio_methodology.md) · [Limitations](LIMITATIONS.md)
...
4. **History says these crashes are selective, not uniform:** in 2000 to 2002 the
   Nasdaq fell 78% while value fell 34%, a concentration crash, not a market crash.
...
exact mix actually returned over the trailing year against SPY, so the upside case
isn't only ever hypothetical, and then the repricing simulation itself: four
conditional scenarios (no-bubble trend continuation, 2022-style, 2008-style,
dot-com-style) plus a menu of upside kept versus downside risked. A bonus panel at the
end plots a rolling 252-day beta timeline, so you can see roughly when your own
portfolio became an AI fund.

Each stage only opens once you act on the one before it. The app is built to be
...
peak-to-trough drawdown, documented at roughly −25%. It returned **−24.5%**, the
residual explained by using daily closes rather than intraday extremes. If a pipeline
can't reproduce a known number, nothing downstream can be trusted. This gate is
re-asserted at the start of every module and on app startup.
...
and the remaining (100−X)% were uncorrelated with it, beta would be roughly X/100. When
beta *exceeds* the direct weight, the rest of the portfolio isn't neutral: it's full of
...
|---|---|---|---|---|
| Dot-com (Mar 2000 to Oct 2002) | Nasdaq **−77.9%** | −49.1% | −34.1% | *no coverage* |
| GFC (Oct 2007 to Mar 2009) | −54%-range | **−56.8%** | −59.8% | TLT **+26.4%** (return) |
| 2022 (Jan to Oct 2022) | QQQ −34.8% | −24.5% | −17.0% | TLT −34.9% |

Three windows show three different failure modes of diversification: a
**concentration crash** in 2000, where value barely felt it; a **systemic crash** in
2008, where equity diversification failed and bonds saved you; and a **rate shock** in
2022, where stocks *and* bonds fell together.
...
Projected outcome is roughly `β_AI × AI_shock + β_rest × rest_shock`, with every shock
read **programmatically from Module 2's output table**, never hand-typed. Scenarios:
trend continuation ("no bubble"), 2022-style, 2008-style, and dot-com-style repricings.
...
The app shares its computation layer (`analysis/factor_lib.py`) with Modules 1 to 3,
and nothing in the app is a reimplementation. Two things go beyond the original
research: it shows realized performance alongside the four conditional scenarios,
meaning what a portfolio's exact mix would have actually returned over the trailing
year against SPY, so the upside isn't only ever hypothetical. And it's a guided,
step-gated flow rather than a wall of charts, where each stage (calculate, compare,
simulate) only reveals once the user has acted on the one before it.
...
dropped in favor of the one finding that's actually true and already verified: the S&P
500 concentration record in Module 1. That's documented in
...
- **Linear beta understates crash correlations.** Correlations rise in drawdowns, so
  calm-period betas likely understate real crash exposure. That works against the
  headline findings, not for them.
- **The shock mapping is an analogy, not an identity.** "Nasdaq in 2000 ≈ the AI basket
  ...
  The 2008 mapping is the weakest of the three, since tech wasn't that crash's epicenter.
...
- **A concentration-growth metric was tested for the app and didn't hold up** (see
  Module 4 above). It's included here for transparency, not hidden.
